...
- (void)testHelloNow {
    NSCalendarIdentifier currentID = [NSCalendar currentCalendar].calendarIdentifier;
    XCTAssertEqual([self.testClient helloMessageByCalendarIdentifier:currentID], [self.testClient helloMessage], @"the hello mesasge is wrong");
}
...
- (void)testHelloHourInvalid {
    ...
    XCTAssertThrows([self.testClient helloMessageByCalendarIdentifier:@"abc"], @"CalendarIdentifierInvalid should throw exception");
}
